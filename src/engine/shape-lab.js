/* shape-lab.js — "Claude cloud": a first-person flythrough of an INFINITE field
   ...
   Registers <shape-lab>. ES module — imports three directly.
   ...
   Property: projects (array) — replaces the placeholder project list. */
  ...
  class ShapeLab extends HTMLElement {
    ...
    set projects(list) {
      ...
      if (this._tiles) {
        for (var i = 0; i < this._tiles.length; i++) {
          var t = this._tiles[i];
          t.project = list[t.cell % list.length];
        }
      }
    }
    ...
    _init() {
      ...
      for (var i = 0; i < MAX_TILES; i++) {
        ...
        this._tiles.push({
          project: this._projects[ci % this._projects.length],
    ...
    _reproject(t) {
      ...
      this._projCursor++;
      t.project = this._projects[ci % this._projects.length];
      t.cell = ci;
    }
